...
fn batch_file_and_namespace_directory() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        ["new_module", "new_ns.sub", "new_module"]
    );
...
fn batch_is_grouped_by_owning_workspace() -> anyhow::Result<()> {
    ...
    assert_eq!(replacement_texts(&b_edits), ["b.new_b", "new_b"]);
    assert!(changes.is_empty());
...
fn open_files_are_candidates() -> anyhow::Result<()> {
    ...
    server.send_notification::<DidOpenTextDocumentNotification>(DidOpenTextDocumentParams {
        text_document: TextDocumentItem {
            ...
            version: 1,
            text: "import old_pkg.sub as sub\nprint(sub.x)\n".to_string(),
        },
    });

    let mut changes = rename_changes(&mut server, &[("old_pkg", "new_pkg")]);
    for uri in [first_cell_uri, second_cell_uri, untitled_uri] {
